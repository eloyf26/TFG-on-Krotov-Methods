...
def test_parallel_map():
    ...
    ys_serial = mykrotov.parallelization.serial_map(
        func, xs, task_args=(2,), task_kwargs={'use_sqrt': True}
    )
    assert np.max(np.abs(ys_serial - expected)) < 1e-15

    mykrotov.parallelization.USE_LOKY = True
    ys_parallel = mykrotov.parallelization.parallel_map(
        func, xs, task_args=(2,), task_kwargs={'use_sqrt': True}
    )
    assert np.max(np.abs(ys_parallel - expected)) < 1e-15

    mykrotov.parallelization.USE_LOKY = False
    ys_parallel = mykrotov.parallelization.parallel_map(
...
def test_parallel_map_fw_prop_step_loky(transmon_xgate_system):
    ...
    mykrotov.parallelization.USE_LOKY = True
    log = io.StringIO()
    opt_result_loky = mykrotov.optimize_pulses(
        objectives,
        pulse_options,
        tlist,
        propagator=mykrotov.propagators.expm,
        chi_constructor=mykrotov.functionals.chis_re,
        info_hook=partial(mykrotov.info_hooks.print_debug_information, out=log),
        iter_stop=1,
        skip_initial_forward_propagation=True,
        parallel_map=(
            mykrotov.parallelization.parallel_map,
            mykrotov.parallelization.parallel_map,
            mykrotov.parallelization.parallel_map_fw_prop_step,
        ),
    )
    ...
    try:
        opt_result = mykrotov.optimize_pulses(
            objectives,
            pulse_options,
            tlist,
            propagator=mykrotov.propagators.expm,
