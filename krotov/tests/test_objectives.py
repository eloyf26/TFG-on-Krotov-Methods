...
def objective_liouville():
    ...
    L = mykrotov.objectives.liouvillian(H, c_ops=[])
    ket00 = ket((0, 0))
    ket11 = ket((1, 1))
    obj = mykrotov.Objective(
...
def test_krotov_objective_initialization(transmon_ham_and_states):
    ...
    obj = mykrotov.Objective(initial_state=psi0, target=psi1, H=H)
    assert obj.H == H
    assert obj.initial_state == psi0
    assert obj.target == psi1
    assert obj == mykrotov.Objective(H=H, initial_state=psi0, target=psi1)
...
def test_adoint_objective(transmon_ham_and_states, objective_with_c_ops):
    ...
    H, psi0, psi1 = transmon_ham_and_states
    target = mykrotov.Objective(initial_state=psi0, target=psi1, H=H)
...
def test_adoint_objective_with_no_target(transmon_ham_and_states):
    ...
    target = mykrotov.Objective(initial_state=psi0, target=None, H=H)
    adjoint_target = target.adjoint()
...
def test_objective_custom_target(objective_liouville):
    ...
    mykrotov.objectives.Objective.reset_symbol_counters()
    obj = copy.deepcopy(objective_liouville)
    obj.target = OrderedDict([('func', 'PE'), ('val', 1)])
    obj_dag = obj.adjoint()
    assert obj_dag.target == obj.target
    with pytest.raises(ValueError):
        mykrotov.objectives._adjoint(obj.target, ignore_errors=False)
...
def test_adjoint_of_invalid_nested_list():
    ...
    with pytest.raises(ValueError) as exc_info:
        mykrotov.objectives._adjoint(H, ignore_errors=False)
    assert "expected format" in str(exc_info.value)
    assert mykrotov.objectives._adjoint(H, ignore_errors=True) == H
...
def test_invalid_objective(transmon_ham_and_states):
    ...
    H, psi0, psi1 = transmon_ham_and_states
    with pytest.raises(ValueError) as exc_info:
        mykrotov.Objective(initial_state=psi0.full, target=psi1, H=H)
    assert "Invalid initial_state" in str(exc_info.value)
    with pytest.raises(ValueError) as exc_info:
        mykrotov.Objective(initial_state=None, target=psi1, H=H)
    assert "Invalid initial_state" in str(exc_info.value)
    with pytest.raises(ValueError) as exc_info:
        mykrotov.Objective(initial_state=psi0, target=psi1, H=tuple(H))
    assert "Invalid H" in str(exc_info.value)
    with pytest.raises(ValueError) as exc_info:
        mykrotov.Objective(initial_state=psi0, target=psi1, H=None)
    assert "Invalid H" in str(exc_info.value)
    with pytest.raises(ValueError) as exc_info:
        mykrotov.Objective(initial_state=psi0, target=psi1, H=H[0].full)
    assert "Invalid H" in str(exc_info.value)
    with pytest.raises(ValueError) as exc_info:
        mykrotov.Objective(initial_state=psi0, target=psi1, H=H, c_ops=H[0])
...
def test_objective_mesolve_propagate(transmon_ham_and_states, tlist_control):
    ...
    res2 = target.propagate(tlist, propagator=mykrotov.propagators.expm)
    ...
    res2 = target.propagate(
        tlist, e_ops=e_ops, propagator=mykrotov.propagators.expm
    )
...
def test_plug_in_array_controls_as_func():
    ...
    H_with_funcs = mykrotov.objectives._plug_in_array_controls_as_func(
...
def test_gate_objectives_single_qubit_gate():
    ...
    objectives = mykrotov.objectives.gate_objectives(basis, gate, H)
    assert objectives == [
        mykrotov.Objective(initial_state=basis[0], target=(1j * basis[1]), H=H),
        mykrotov.Objective(initial_state=basis[1], target=(-1j * basis[0]), H=H),
    ]
...
def test_gate_objectives_shape_error():
    ...
    with pytest.raises(ValueError) as exc_info:
        mykrotov.objectives.gate_objectives(basis, gate, H)
...
def test_ensemble_objectives(transmon_ham_and_states):
    ...
    H, psi0, psi1 = transmon_ham_and_states
    objectives = [
        mykrotov.Objective(initial_state=psi0, target=psi1, H=H),
        mykrotov.Objective(initial_state=psi1, target=psi0, H=H),
    ]
    (H0, (H1, eps)) = H
    Hs = [[H0, [mu * H1, eps]] for mu in [0.95, 0.99, 1.01, 1.05]]
    ensemble_objectives = mykrotov.ensemble_objectives(objectives, Hs)
...
def test_gate_objectives_pe():
    ...
    objectives = mykrotov.gate_objectives(basis, 'PE', H)
    assert len(objectives) == 4
    bell_basis_states = bell_basis(basis)
    for state in bell_basis_states:
        assert isinstance(state, qutip.Qobj)
    for i in range(4):
        expected_objective = mykrotov.Objective(
            initial_state=bell_basis_states[i], target='PE', H=H
        )
        assert objectives[i] == expected_objective
    assert mykrotov.gate_objectives(basis, 'perfect_entangler', H) == objectives
    assert mykrotov.gate_objectives(basis, 'perfect entangler', H) == objectives
    assert mykrotov.gate_objectives(basis, 'Perfect Entangler', H) == objectives
    with pytest.raises(ValueError):
        mykrotov.gate_objectives(basis, 'prefect(!) entanglers', H)
...
def test_liouvillian():
    ...
    assert (
        mykrotov.objectives.liouvillian(H[0], c_ops)
    ...
    with pytest.raises(ValueError):
        mykrotov.objectives.liouvillian(tuple(H), c_ops)
...
def two_qubit_liouvillian():
    ...
    c_ops = [tensor(sigmam(), identity(2)), tensor(identity(2), sigmam())]
    return mykrotov.objectives.liouvillian(H, c_ops)
...
def test_gate_objectives_3states(two_qubit_liouvillian):
    ...
    objectives = mykrotov.objectives.gate_objectives(
    ...
    with pytest.raises(ValueError):
        mykrotov.objectives.gate_objectives(
...
def test_transmon_3states_objectives():
    ...
    weights = [20, 1, 1]
    objectives = mykrotov.gate_objectives(
        basis,
...
def test_summarize_component_direct():
    ...
    H = ['H0', ['H1', 2j]]
    res = mykrotov.objectives._summarize_component(H, 'op')
    expected = '[H0, [H1, 2j]]'
    assert res == expected

    with pytest.raises(ValueError):
        mykrotov.objectives._summarize_component(H, 'invalid')

    ket0 = qutip.ket('0')
    ket1 = qutip.ket('1')
    ket2 = copy.deepcopy(ket0)
    assert mykrotov.objectives._summarize_component(ket0, 'state') == '|Ψ₀(2)⟩'
    assert mykrotov.objectives._summarize_component(ket1, 'state') == '|Ψ₁(2)⟩'
    assert mykrotov.objectives._summarize_component(ket2, 'state') == '|Ψ₂(2)⟩'
    mykrotov.objectives.Objective.reset_symbol_counters()
    assert mykrotov.objectives._summarize_component(ket2, 'state') == '|Ψ₀(2)⟩'

    H = qutip.sigmaz()
    assert mykrotov.objectives._summarize_component(H, 'op') == 'H₀[2,2]'
    assert mykrotov.objectives._summarize_component(H, 'lindblad') == 'L₀[2,2]'
...
def test_objective_pickle(objective_with_c_ops):
    ...
    with io.BytesIO() as buffer:
        ...
        pickler.dispatch_table[
            mykrotov.objectives.Objective
        ] = mykrotov.objectives._Objective_reduce
        pickler.dump(obj1)
        buffer.seek(0)
    ...
    assert isinstance(obj2.H[1][1], mykrotov.objectives._ControlPlaceholder)
...
def test_recursive_eq_dict():
    ...
    with pytest.raises(ValueError):
        d1 == d2
    assert mykrotov.objectives._recursive_eq(d1, d2)
