...
import mykrotov
from mykrotov.conversions import (
    discretize,
    extract_controls,
    extract_controls_mapping,
    pulse_options_dict_to_list,
)
from mykrotov.shapes import qutip_callback
...
def test_conversion_control_pulse_inverse():
    ...
    blackman = qutip_callback(mykrotov.shapes.blackman, t_start=0, t_stop=10)

    pulse_orig = mykrotov.conversions.control_onto_interval(
        discretize(blackman, tlist)
    )

    control = mykrotov.conversions.pulse_onto_tlist(pulse_orig)
    pulse = mykrotov.conversions.control_onto_interval(control)
...
def test_discretize():
    ...
    control = partial(mykrotov.shapes.blackman, t_start=0, t_stop=10)
    ...
    control = qutip_callback(mykrotov.shapes.blackman, t_start=0, t_stop=10)
...
def test_discretization_as_float():
    ...
    u = H[1][1]
    objectives = [
        mykrotov.Objective(initial_state=qutip.Qobj(), target=None, H=H)
    ]
    tlist = np.linspace(0, 10, 100)

    res = mykrotov.optimize._initialize_krotov_controls(
...
def test_initialize_krotov_controls():
    ...
    T = 10
    blackman = qutip_callback(mykrotov.shapes.blackman, t_start=0, t_stop=T)
    H = ['H0', ['H1', blackman]]
    ...
    (
        ...
        lambda_vals,
        shape_arrays,
    ) = mykrotov.optimize._initialize_krotov_controls(
...
def test_extract_controls_with_arrays():
    ...
    objectives = [
        mykrotov.Objective(initial_state=psi0, target=psi_tgt, H=H1),
        mykrotov.Objective(initial_state=psi0, target=psi_tgt, H=H2),
    ]
...
def test_extract_controls():
    ...
    objectives = [
        mykrotov.Objective(initial_state=X, target=Y, H=H1),
        mykrotov.Objective(initial_state=Y, target=X, H=H1),
    ]
    controls = extract_controls(objectives)
    ...
    objectives = [
        mykrotov.Objective(initial_state=X, target=Y, H=H1),
        mykrotov.Objective(initial_state=Y, target=X, H=H2),
        mykrotov.Objective(initial_state=Y, target=X, H=H3),
    ]
    controls = extract_controls(objectives)
...
def test_control_tlist_calculation():
    ...
    midpoints = mykrotov.conversions._tlist_midpoints(tlist)
