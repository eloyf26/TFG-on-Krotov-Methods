...
def test_valid_version():
    ...
    assert parse_version(mykrotov.__version__) >= parse_version("0.1.0")
...
def test_complex_control_rejection():
    ...
    def S(t):
        ...
        return mykrotov.shapes.flattop(
            t, t_start=0, t_stop=5, t_rise=0.3, t_fall=0.3, func='sinsq'
        )

    H = [H0, [H1, eps0]]

    objectives = [mykrotov.Objective(initial_state=psi0, target=psi1, H=H)]
    ...
    with pytest.raises(ValueError) as exc_info:
        mykrotov.optimize_pulses(
            objectives,
            pulse_options,
            tlist,
            propagator=mykrotov.propagators.expm,
            chi_constructor=mykrotov.functionals.chis_re,
            iter_stop=0,
        )
    ...
    def S2(t):
        """Shape function for the field update"""
        return 2.0 * mykrotov.shapes.flattop(
            t, t_start=0, t_stop=5, t_rise=0.3, t_fall=0.3, func='sinsq'
        )
...
def test_reject_invalid_shapes():
    ...
    def S_complex(t):
        """Shape function for the field update"""
        return 1j * mykrotov.shapes.flattop(
            t, t_start=0, t_stop=5, t_rise=0.3, t_fall=0.3, func='sinsq'
        )

    def S_negative(t):
        """Shape function for the field update"""
        return -1 * mykrotov.shapes.flattop(
            t, t_start=0, t_stop=5, t_rise=0.3, t_fall=0.3, func='sinsq'
        )

    def S_large(t):
        """Shape function for the field update"""
        return 2 * mykrotov.shapes.flattop(
            t, t_start=0, t_stop=5, t_rise=0.3, t_fall=0.3, func='sinsq'
        )
...
def test_zero_iterations(iter_stop, simple_state_to_state_system):
    ...
    with io.StringIO() as log_fh:
        ...
        result = mykrotov.optimize_pulses(
            objectives,
            ...
            info_hook=mykrotov.info_hooks.print_table(
                J_T=mykrotov.functionals.J_T_re, out=log_fh
            ),
            iter_stop=iter_stop,
...
def test_continue_optimization(
    ...
    with open(logfile, 'w', encoding='utf8') as log_fh:
        ...
        with caplog.at_level(logging.WARNING):
            oct_result1 = mykrotov.optimize_pulses(
                objectives,
                pulse_options=pulse_options,
                tlist=tlist,
                propagator=mykrotov.propagators.expm,
                ...
                check_convergence=mykrotov.convergence.Or(
                    mykrotov.convergence.check_monotonic_error,
                    mykrotov.convergence.dump_result(dumpfile, every=2),
                ),
                iter_stop=3,
                skip_initial_forward_propagation=True,
        ...
        oct_result2 = mykrotov.optimize_pulses(
            objectives,
            pulse_options=pulse_options,
            tlist=tlist,
            propagator=mykrotov.propagators.expm,
        ...
        oct_result3 = mykrotov.optimize_pulses(
            objectives,
            pulse_options=pulse_options,
            tlist=tlist,
            propagator=mykrotov.propagators.expm,
        ...
        oct_result4 = mykrotov.optimize_pulses(
            objectives,
            pulse_options=pulse_options,
            tlist=tlist,
            propagator=mykrotov.propagators.expm,
    ...
    oct_result5 = mykrotov.optimize_pulses(
        ...
        chi_constructor=mykrotov.functionals.chis_re,
        store_all_pulses=True,
        info_hook=mykrotov.functionals.J_T_re,
        check_convergence=mykrotov.convergence.Or(
            mykrotov.convergence.check_monotonic_error,
            mykrotov.convergence.dump_result(dumpfile, every=2),
        ),
