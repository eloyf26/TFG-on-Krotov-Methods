...
def oct_result_20(request):
    ...
    return mykrotov.result.Result.load(filename)
...
def test_invalid_dump_result(oct_result_20):
    with pytest.raises(ValueError):
        mykrotov.convergence.dump_result('oct_result.dump', every=0)
    filename = "non_existent_folder/oct_result.dump"
    dump_result = mykrotov.convergence.dump_result(filename)
...
def test_dump_result_overwrite(oct_result_5, oct_result_10, tmpdir):
    ...
    dump_result = mykrotov.convergence.dump_result(filename, every=5)

    response = dump_result(oct_result_5)
    assert response is None
    assert os.path.isfile(filename)
    result = mykrotov.result.Result.load(filename)
