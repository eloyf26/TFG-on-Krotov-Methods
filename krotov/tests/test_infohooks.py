...
import mykrotov
from mykrotov.info_hooks import print_table
...
def test_infohook_chaining(transmon_ham_and_states):
    ...
    with contextlib.redirect_stdout(stdout):
        oct_result = mykrotov.optimize_pulses(
            [obj],
            pulse_options=pulse_options,
            tlist=tlist,
            propagator=mykrotov.propagators.expm,
            chi_constructor=mykrotov.functionals.chis_re,
            info_hook=mykrotov.info_hooks.chain(print_fidelity, print_messages),
    ...
    with io.StringIO() as log_fh:
        ...
        mykrotov.optimize_pulses(
            ...
            info_hook=mykrotov.info_hooks.print_table(
                J_T=mykrotov.functionals.J_T_re,
                col_formats=(
                    '%8d',
                    '%12.4e',
                    '%12.4e',
