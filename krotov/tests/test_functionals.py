...
def sqrt_SWAP_basis(canonical_basis):
    return mykrotov.functionals.mapped_basis(
...
def cphase_objectives(canonical_basis):
    ...
    return mykrotov.objectives.gate_objectives(
...
def test_mapped_basis_preserves_hs_structure():
    ...
    states = mykrotov.functionals.mapped_basis(qutip_gates.cnot(), basis)
    for state in states:
...
def test_f_tau_with_weights(sqrt_SWAP_basis, cphase_objectives):
    ...
    F = mykrotov.functionals.f_tau(sqrt_SWAP_basis, cphase_objectives)
    ...
    F = mykrotov.functionals.f_tau(sqrt_SWAP_basis, objectives)
...
def test_J_T_ss(sqrt_SWAP_basis, cphase_objectives):
    J = mykrotov.functionals.J_T_ss(sqrt_SWAP_basis, cphase_objectives)
    assert abs(J - 0.25) < 1e-14


def test_J_T_sm(sqrt_SWAP_basis, cphase_objectives):
    J = mykrotov.functionals.J_T_sm(sqrt_SWAP_basis, cphase_objectives)
    assert abs(J - 0.875) < 1e-14


def test_J_T_re(sqrt_SWAP_basis, cphase_objectives):
    J = mykrotov.functionals.J_T_re(sqrt_SWAP_basis, cphase_objectives)
...
def test_J_T_ss_with_weights(sqrt_SWAP_basis, cphase_objectives):
    ...
    J = mykrotov.functionals.J_T_ss(sqrt_SWAP_basis, objectives)
...
def test_J_T_hs_unitary(
    ...
    J_hs = mykrotov.functionals.J_T_hs(sqrt_SWAP_basis, cphase_objectives)
    J_re = mykrotov.functionals.J_T_re(sqrt_SWAP_basis, cphase_objectives)
    ...
    J_hs = mykrotov.functionals.J_T_hs(fw_states_T, cphase_lv_full_objectives)
    J_re = mykrotov.functionals.J_T_re(fw_states_T, cphase_lv_full_objectives)
    assert abs(J_hs - J_re) < 1e-14
    assert J_re != J_re_1
...
def test_chi_hs_transmon(transmon_3states_objectives):
    ...
    χs = mykrotov.functionals.chis_hs(fw_states_T, objectives, None)
...
def test_chi_ss(iswap_state_objectives, canonical_basis):
    χs = mykrotov.functionals.chis_ss(
...
def test_chi_sm(iswap_state_objectives, canonical_basis):
    χs = mykrotov.functionals.chis_sm(
        fw_states_T=canonical_basis,
        objectives=iswap_state_objectives,
        tau_vals=[1, 0.5 * (1 + 1j), 0.5 * (1 + 1j), 1],
    )
...
def test_chi_re(iswap_state_objectives, canonical_basis):
    χs = mykrotov.functionals.chis_re(
